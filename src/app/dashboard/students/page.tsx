...
import { Search, UserPlus, ArrowLeft, MapPin, GraduationCap, BookOpen } from "lucide-react";
...
export default function StudentsDirectoryPage() {
  ...
  useEffect(() => {
    async function fetchStudents() {
      ...
      try {
        ...
        const response = await fetch("/api/profiles?role=student&limit=100", {
  ...
  const handleConnect = async (studentUserId: string) => {
    if (!session?.user) return;
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-muted-foreground">
                Connect with current students at your university
              </p>
            </div>
            <Link href="/dashboard/alumni">
              <Button variant="outline">View Alumni Directory</Button>
            </Link>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              placeholder="Search by major, location, or interests..."
              value={searchTerm}
                ...
                <CardContent className="space-y-3">
                  {person.major && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        {person.major}
                        {person.graduationYear && ` (Expected ${person.graduationYear})`}
                      </span>
                    </div>
                  )}
                  {person.location && (
